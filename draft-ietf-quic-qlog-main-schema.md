...
occurred. The format of the value is governed by the TimeFormat, which MAY be
included in the "time_format" field.
...
absolute:
: A full absolute timestamp. This approach uses the
...
relative:
: A relative timestamp based on some relative time base.  The log SHOULD specify
  a "reference_time" timestamp, either by providing it explicitly in the
  "reference_time" field in "common_fields" ({{common-fields}}) or logging an
...
: A relative timestamp based on an opaque monotonically nondecreasing clock.
  The "reference_time" field MUST NOT be applied to this format, since it based
...
There is no default base clock for formats that rely on a sharable clock. Tools
SHOULD NOT assume the ability to derive the absolute Unix timestamp from qlog
traces. Tools should not rely events to be be consistent across traces, even
those generated by the same logging endpoint. For reasons of privacy, a more
privacy-preserving reference timestamp could be chosen.


The "absolute" format is good for stateless loggers. The other formats suit
stateful loggers.

Formats that use smaller values can produce smaller-sized file and can be easier
